...
namespace CNTK
{
    ...
    protected:
        PrimitiveFunction(const std::vector<Variable>& blockInputs, const std::vector<Variable>& blockOutputs, Dictionary&& functionConfig, const std::wstring& functionName = L"")
            : Function(blockInputs, blockOutputs, std::move(functionConfig), functionName, GenerateUid(PrimitiveOpType::Block)), m_op(PrimitiveOpType::Block)
        {}
