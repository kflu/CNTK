...
namespace CNTK {
    ...
    public:
        ...
        UserFunction(const std::vector<Variable>& inputs, 
                const std::vector<std::tuple<NDShape, DataType, Function*, std::vector<Axis>>>& outputs, 
                const std::wstring& name, const std::wstring& opName)
        ...
        virtual std::vector<Variable> GetOutputVariables(
                const std::vector<std::tuple<NDShape, DataType, Function*, std::vector<Axis>>>& outputDescription)
        { 
            std::vector<Variable> outputs;

            for (auto it: outputDescription)
            {
                NDShape shape  = std::get<0>(it);
                DataType dtype = std::get<1>(it);
                Function* func = std::get<2>(it);
                std::vector<Axis> axes = std::get<3>(it);
                outputs.push_back(OutputVariable(shape, dtype, func, axes));
            }
